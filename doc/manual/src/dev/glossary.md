...
| **Backend** [L]                | Motor de cálculo detrás de `IComputeBackend` (subproceso `scilab-cli`, `call_scilab`, …). |
| **Bridge** [L]                 | El delegado numérico de alto nivel (`ScilabBridge`), que implementa `ISimSession`. |
...
| **Gramática** [H]              | Las reglas R0–R7 que validan un grafo.                                       |
...
| **IComputeBackend** [L]        | Interfaz que abstrae el motor de cálculo (subproceso Scilab, `call_scilab`, …). |
| **INodeRenderer** [L]          | Interfaz que abstrae el render del canvas; su implementación es `NativeNodeRenderer`. |
...
| **R0..R7** [H/L]               | Las ocho reglas de gramática.                                                 |
...
| **Stateful** [L]               | Nodo que tiene memoria (Integrator, Low-Pass Filter, …).                     |
